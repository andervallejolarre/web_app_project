            ...
            const plantTypeData = await plantType.findOne({ type: plantData?.type }).select('-_id -__v');
                ...
                if (plantTypeData && weather && plantData) {
                    ...
                    const updatedValues = {
                        ...
                        stress: plantData.stress,
                        firstVisit:plantData.firstVisit
                    }
                    ...
                    if (Date.now() >= minUpdateTime) {
                        ...
                        await plant.updateOne({ owner: decoded.id }, { progress: calc, level: level, stress: stress, updated: Date.now(), firstVisit: false })
                        //Update object with updated values
                        updatedValues.progress = calc;
                        updatedValues.level = level;
                        updatedValues.stress = stress;
                        updatedValues.firstVisit = false;

                    }

                    res.send({ ok: true, payload: { final: finalMessage, update: updatedValues } });
                } else {
